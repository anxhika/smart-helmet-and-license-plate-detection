...
MIRROR_OUTPUT_DIR = PROJECT_ROOT / "yolo11" / "outputs" / "images"
LEGACY_STATUS_TEXT = "Not"


def _collect_image_files(source: Path) -> List[Path]:
    if source.is_file():
...
def process_images(
    ...
    files = _collect_image_files(source)
    ...
    for idx, image_path in enumerate(files):
        ...
        plate_number = None
        if lp_count > 0:
            lp_crop = _extract_best_lp_crop(pred, pred.orig_img)
        ...
        image_name = f"Det_{idx}.png"
        txt_name = f"Det_{idx}.txt"
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run YOLO11 image inference with YOLOv5-style outputs.")
    parser.add_argument("--image", default=None, help="Path to one image file (backward-compatible).")
    parser.add_argument("--input", default=None, help="Path to image file or image folder.")
...
if __name__ == "__main__":
    ...
    source = args.input or args.image
    if not source:
        raise ValueError("Provide --input <file_or_folder> or --image <file>.")
